    ...
    <div v-if="!isEmpty" :id="`person-${props.compData.id}`">
      <div :id="`head-warp-${props.compData.id}`">
        <div :id="`head-${props.compData.id}`" class="head" :style="headStyle">
          <div
            :id="`face-${props.compData.id}`"
            :ref="`face-${props.compData.id}`"
            class="face"
            :style="faceStyle"
          ></div>
        </div>
...
const drop = (e) => {
  ...
  let targetId =
    e.target.id.indexOf('-') > -1 ? e.target.id.split('-')[1] : e.target.id
