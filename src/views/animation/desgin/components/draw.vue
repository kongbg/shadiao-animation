<template>
  <div :class="['dh-draw__warpper', drawStore.mode]">
    <div v-show="!isStart" :class="['tools', drawStore.mode]">
      ...
      <el-button @click="preLoadAudio" type="primary" size="small"
        >预加载音频并播放</el-button
      >
      <el-button @click="start" id="play" type="primary" size="small"
        >播放</el-button
      >
      <el-button
        @click="preview"
        v-show="drawStore.mode == 'desgin'"
        type="primary"
        size="small"
        >预览</el-button
      >
      ...
      >
      <el-button
        @click="setEditActionId"
        v-show="drawStore.editActionId"
        type="primary"
        size="small"
        >退出设置动画</el-button
      >
    ...
    <div
      id="dh-draw-content"
      ref="dh-draw-content"
      class="dh-draw-content"
      @dragover="(e) => dragover(e, drawStore.dragElType)"
      @drop="drop"
    >
      <comp-warpper2
...
const currentConfInfo = computed(() => {
  return drawStore.drawConfigs.confs.length
...
let comps = computed(() => {
  if (drawStore.drawConfigs.confs.length > drawStore.currentConfIndex) {
    let comps = drawStore.drawConfigs.confs.length
      ? drawStore.drawConfigs.confs[drawStore.currentConfIndex].comps
      : []
    comps.forEach((item) => {
  ...
  while (count < confs.length) {
    ...
    while (count2 < actions.length) {
      ...
      while (count3 < actions2.length) {
        ...
        let speakActions = info.actions.filter((res) =>
          res.type.includes('speak')
        )
        let count4 = 0
        while (count4 < speakActions.length) {
          let res = speakActions[count4]
          let url = await startText(
            res.content,
