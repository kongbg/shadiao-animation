<template>
    <div class="numes-list">
        ...
        <el-collapse class="collapse videoType" v-model="drawStore.activeVideoTypes">
            <div v-if="!drawStore.videoTypes.length">空</div>
            <el-collapse-item class="item__warpper" :title="item.name" :name="item.id"
                v-for="(item, vindex) in drawStore.videoTypes" :key="item.id">
                ...
                <div class="texts">
                    <div :class="['t-item', drawStore.videoId == child.id ? 'active' : '']"
                        v-for="(child, index) in  item.list" :key="child.id" >
                        <span>{{ index + 1 }}</span>
                        <span class="name" @click="videoClick(child)">{{ child.name }}</span>

                        <el-popover popper-class="more-action" placement="bottom" trigger="click">
                            <div class="actions">
                                <div class="item">
                                    <el-button type="primary" size="small" @click="initSpeaks(child)">配音</el-button>
                                </div>
                                <div class="item">
                                    <el-button type="primary" size="small" @click="preLoadAudio()">生成配音</el-button>
                                </div>
                                <div class="item">
                                    <el-button type="primary" size="small" @click="deleteVideoByid(child)">删除</el-button>
                                </div>
                            </div>
                            <template #reference>
                                <el-icon>
        ...
        <el-collapse class="collapse" v-model="activeNames">
            <el-collapse-item class="item__warpper" :title="item.name" :name="item.id" v-for="item in drawComps"
                :key="item.id">
        ...
        <el-dialog v-model="speaksDialog" class="speaks-dialog" :close-on-click-modal="false" title="人物配音" width="50%">
                        ...
                        <div class="item" v-for="item in videoInfo.speaks">
                            ...
                            <div class="voice">
                                <el-select v-model="item.voice" placeholder="请选择配音">
                                    <el-option v-for="item in voiceList" :key="item.ShortName" :label="item.label"
            ...
            <template #footer>
                <div class="dialog-footer">
                    ...
                    <el-button type="primary" @click="saveSpeakConf">
                        确认
                    </el-button>
                </div>
            </template>
...
import { saveAudios, getAudios, upload, getAudiosConf } from '@/api/audio/index.js'
import { addVideoType, getVideoType, deleteVideoType } from '@/api/videoType/index.js'
...
async function getVideoTypes() {
    let [err, data] = await getVideoType();
    debugger
    if (!err) {
...
const handleChange = (data, index, videoType) => {
    ...
    reader.onload = async (e) => {
        ...
        // 刷新所有视频合集分类
        await getVideoTypes();
        let info = drawStore.videoTypes[index].list.find(item => item.name == name);
        if (info) {
...
async function play(playLoad) {
    ...
    audio.addEventListener('canplaythrough', () => {
        ...
        let renameFile = new File([blob], `${id}-3do6cw1u-${duration}.wav`, { type: 'audio/wav' })
        const formData = new FormData();
        formData.append('file', renameFile);
        ...
        item.content.forEach(item=> {
            ...
            if (!info) {
                if (speak == '背景') {
                    ...
                    let urls = ['http://127.0.0.1:3000/uploads/images/is0ncu9t.jpg', 'http://127.0.0.1:3000/uploads/images/cqn1oqwk.jpeg', 'http://127.0.0.1:3000/uploads/images/hbwmzar0.jpg','http://127.0.0.1:3000/uploads/images/a9j5e4wm.jpg', 'http://127.0.0.1:3000/uploads/images/8z9z443h.jpg']
        ...
        list: [
                {
                    ...
                    type: 'background',
                    name: '背景1',
                    url: 'http://127.0.0.1:3000/uploads/images/is0ncu9t.jpg',
                    width: 630,
                    height: 408,
                    x: 100,
                    y: 100,
                ...
                },
                {
                    ...
                    type: 'background',
                    name: '背景2',
                    url: 'http://127.0.0.1:3000/uploads/images/a9j5e4wm.jpg',
                    width: 630,
                    height: 408,
                    x: 100,
                    y: 100,
                ...
                },
                {
                    ...
                    type: 'background',
                    name: '背景3',
                    url: 'http://127.0.0.1:3000/uploads/images/8z9z443h.jpg',
                    width: 630,
                    height: 408,
                    x: 100,
                    y: 100,
                ...
                },
                {
                    ...
                    type: 'background',
                    name: '背景4',
                    url: 'http://127.0.0.1:3000/uploads/images/hbwmzar0.jpg',
                    width: 630,
                    height: 408,
                    x: 100,
                    y: 100,
