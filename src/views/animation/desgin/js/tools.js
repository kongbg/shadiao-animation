...
export function createSprite(data) {
    let { id, type, name, x = 0, y = 0, scale = 1, width, height, zIndex = 1, anchor = 0.5, history } = data;
...
export function createContainer(options) {
    let { name, id, type, scalex=1, scaley=1, history, zIndex } = options;
    ...
    container.scale.x = scalex; // 水平方向缩放倍数为1.5
...
// 创建边框
export function createGraphics(options) {
...
export function createComp(data) {
    ...
    let { id, name, type, x, y, width, height, scalex=1, scaley=1, children = [], anchor, screen, history, zIndex } = data;
    // 创建容器
    ...
    comp.screen = screen;
    comp.isDeleteIng = false;

    // 真实内容容器
    let imgContainer = createContainer(
        {
            name: 'imgContainer',
            id: `${generateUniqueID()}`,
            type: 'imgContainer',
    ...
    if (children.length) {
        children.forEach(child => {
            let { id: cid, name: cname, type: ctype, x: cx = 0, y: cy = 0, width: cwidth, height: cheight, zIndex: czIndex } = child;
            let ckey = cid;
        ...
        let key = id;
        // let zIndex = type == 'background' ? 1 : 11
        let sprite = createSprite(
            {
                id: generateUniqueID(),
                type,
    ...
    let throttleFn = throttle(function(event) {
        ...
        // 检查点击的目标元素是否是指定的 DOM 元素或其子元素
        if (!(targetElement.closest('#game') || targetElement.closest('.pannelPeer'))) {
            // 点击的目标元素不是指定的 DOM 元素或其子元素，执行相应操作
            // console.log('点击了指定 DOM 以外的元素');
            Bus.$emit('selectComp')
        }
    }, 100)
    // 监听整个文档的点击事件
    document.addEventListener('click', throttleFn);
...
function enableDrag (sprite) {
    ...
    // 设置sprite 点击事件
    sprite.on("pointerdown", onDragStart.bind(sprite))
...
function onDragStart(event) {
    ...
    this.dragging = true;
    this.initialPosition = this.data.getLocalPosition(
        this.parent
    );
    
    this.initialRotation = this.rotation;
    this.initialScale = new PIXI.Point(
        this.scale.x,
        this.scale.y
    );
    // 点击处坐标跟锚点的差值
    ...
    // 目标起始锚点坐标
    if(!this.startpos) this.startpos = { x: this.x, y: this.y };
}
function onDragEnd(event) {
    ...
    if (this.dragging) {
        ...
        const dr = this.rotation - this.initialRotation;
        ...
        if (dx !== 0 || dy !== 0) {
            this.history.record({
                undoInfo: {
                    params: { 
                        x: this.initialPosition.x - this.diff.x,
                        y: this.initialPosition.y - this.diff.y,
                    },
                    undo: ({x, y}) => {
                        this.x = x;
                        this.y = y;
        ...
        if (dsx !== 0 || dsy !== 0) {
            this.history.record({
                ...
                redoInfo: {
                    params: { 
                        x: this.initialScale.x + dsx,
                        y: this.initialScale.y + dsy
                    },
                    redo: ({x, y}) => {
                        this.scale.set(1, 1);
                    }
    ...
    if (this.dragging) {
        ...
        if (this.data.originalEvent.shiftKey) {
            ...
            const initialDistance = Math.hypot(
                this.initialPosition.x - this.x,
                this.initialPosition.y - this.y
            );
            ...
            // Normal drag
            // 目标新坐标 = 鼠标新位置 - 点击处坐标跟锚点的差值
            this.x = newPosition.x - this.diff.x;
            this.y = newPosition.y - this.diff.y;

            // 限制边界
            if(this.y < this.pivot.y) this.y = this.pivot.y;
            if(this.x < this.pivot.x) this.x = this.pivot.x;
            if(this.y > this.screen.height - this.pivot.y) this.y = this.screen.height - this.pivot.y;
            if(this.x > this.screen.width - this.pivot.x) this.x = this.screen.width - this.pivot.x;
        }
    }
}
...
export function getPersonImageUrl(name, ext = "png") {
    return new URL(`/src/assets/person${name}.${ext}`, import.meta.url).href
}
