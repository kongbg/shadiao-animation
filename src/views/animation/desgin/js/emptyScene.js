...
export default class EmptyScene extends Scene {
                ...
                this.actions.forEach(item => {
                    ...
                    item.actions.forEach((info, oi) => {
                        ...
                        info.actions.forEach((act, ai) => {
                            ...
                            } else if (type[0] == 'head') { // 头部
                                if (type[1] == 'shake') { // 摇头
                                    // 操作对象就是 sprite children 中type == head , face 的 sprite
                                    let imgContainer = sprite.children.find(item => ['imgContainer'].includes(item.type));
                                    let childSprite = imgContainer.children.filter(item => ['head', 'face'].includes(item.type));
                                    if (childSprite.length) {
                                        childSprite.forEach(child => {
                                            let option = {
                                                yoyo: true,
                                                repeat: (gsapOption.duration / 1) + 1,
                                                ease: "linear",
                                                keyframes: [
    ...
    setSpriteLineHeight(id) {
        this.stage.children.forEach(child => {
            if(child.id == id) {
                let borderLine = child.children.find(item=>item.name == "borderLine");
